...
export async function generateMetadata({ params }) {
  ...
  return buildCampusMetadata(inst, "dsa");
}

export default async function CampusDsaPage({ params }) {
  const { slug } = await params;
  const inst = await campusInstitutionForSlug(slug);
  if (!inst) notFound();

  const jsonLd = campusJsonLd(inst, "dsa");
  const breadcrumbItems = campusBreadcrumbItems(inst, "dsa");
  ...
  return (
    <>
      ...
      <CampusSeoIntro breadcrumbItems={breadcrumbItems}
        h1={CAMPUS_SECTIONS.dsa.title(inst)} description={CAMPUS_SECTIONS.dsa.description(inst)} />
      <CampusApp initialTab="dsa" />
    </>
  );
}
