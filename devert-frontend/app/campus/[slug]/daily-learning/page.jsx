...
export async function generateMetadata({ params }) {
  ...
  return buildCampusMetadata(inst, "learning");
}

export default async function CampusDailyLearningPage({ params }) {
  const { slug } = await params;
  const inst = await campusInstitutionForSlug(slug);
  if (!inst) notFound();

  const jsonLd = campusJsonLd(inst, "learning");
  const breadcrumbItems = campusBreadcrumbItems(inst, "learning");
  ...
  return (
    <>
      ...
      <CampusSeoIntro breadcrumbItems={breadcrumbItems}
        h1={CAMPUS_SECTIONS.learning.title(inst)} description={CAMPUS_SECTIONS.learning.description(inst)} />
      <CampusApp initialTab="learning" />
    </>
  );
}
