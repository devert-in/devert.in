...
function CommentsDrawer({ post, user, userData, onClose, onCommented }) {
  ...
  useEffect(() => {
    getDocs(query(collection(db, "pulse_comments"), orderBy("createdAt", "asc")))
      .then(snap => setComments(snap.docs.map(d => ({ id: d.id, ...d.data() })).filter(c => c.postId === post.id)))
      .catch(console.error)
...
function PostCard({ post, user, userData, liked, saved, isFollowing, followBusy, isReposted, repostBusy, onLike, onSave, onComment, onToggleFollow, onToggleRepost, onEdit, onDelete, onGuestAction }) {
  ...
  const handleShare = () => {
    navigator.clipboard.writeText(`${window.location.origin}/pulse`).then(() => {
      setCopied(true);
    ...
    <>
      ...
      >
        ...
        <div className="px-4 pb-3 space-y-2.5 overflow-hidden">
          ...
          {hasExtra && (
            <button onClick={() => setShowDetail(true)} className="font-mono text-[11px] text-neon-cyan/70 hover:text-neon-cyan transition-colors">
              Read more...
            </button>
          )}
      ...
      <AnimatePresence>
        {showDetail && (
          <PostDetailModal post={post} onClose={() => setShowDetail(false)} actionsBar={actionsBar} />
        )}
        {showComments && (
          <CommentsDrawer
...
export default function PulsePage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      getDocs(query(collection(db, "pulse_likes"), orderBy("likedAt", "desc"), limit(200))),
      ...
      setLikedIds(new Set(likeSnap.docs.filter(d => d.data().uid === user.uid).map(d => d.data().postId)));
  ...
  return (
    <>
      ...
      <motion.button
        ...
        whileTap={{ scale: 0.9 }}
        onClick={() => user ? setCreateOpen(true) : setGuestPromptOpen(true)}
        className="fixed z-50 right-5 md:right-10 w-14 h-14 rounded-full flex items-center justify-center"
        style={{
