...
export function CampusRoadmapsTab({ sidebarSlot }) {
  ...
  useEffect(() => {
    ...
    let url = `/campus/${slug}?tab=roadmaps`;
...
function CampusRoadmapsPublicShell({ initialRoadmapSlug }) {
  ...
  const goToRoadmap = useCallback((roadmapId, slug) => {
    ...
    if (typeof window !== "undefined") window.history.pushState(null, "", `/campus/roadmaps/${slug}`);
  }, []);

  const goToTopic = useCallback((topicId) => {
    setScreen((s) => ({ ...s, view: "topic", topicId }));
    if (typeof window !== "undefined" && screen.roadmapSlug) {
      window.history.pushState(null, "", `/campus/roadmaps/${screen.roadmapSlug}?topic=${encodeURIComponent(topicId)}`);
    }
  }, [screen.roadmapSlug]);

  const goToList = useCallback(() => {
    setScreen({ view: "list" });
    if (typeof window !== "undefined") window.history.pushState(null, "", "/campus/roadmaps");
  }, []);
...
function RoadmapCard({ roadmap, progress, onClick }) {
  ...
  return (
    <CampusCard as="a" href={`/campus/roadmaps/${roadmap.slug}`} hover
