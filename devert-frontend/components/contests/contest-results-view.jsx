...
// Shared by the standalone /arena/contests/results route (share-link URLs
// still need a real page) and ContestHub's in-place view.
export function ContestResultsView({ contestId, onBack, onLogin }) {
  ...
  return (
    <main className={rootClass}>
      ...
      <div className="relative max-w-3xl mx-auto">
        <button onClick={() => onBack(contestId)} className="inline-flex items-center gap-1.5 font-mono text-xs text-white/30 hover:text-white/60 transition-colors mb-6">
          ...
          <>
            ...
            {!user && (
              <p className="font-mono text-xs text-white/25 text-center mb-6">
                <button onClick={onLogin} className="text-neon-cyan hover:underline">login</button> to see your personal result
              </p>
            )}
