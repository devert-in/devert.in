...
import { collection, query, orderBy, getDocs, doc, getDoc, setDoc, updateDoc, arrayUnion, increment } from "firebase/firestore";
import { useAuth } from "@/context/AuthContext";
import { useIsWindowed, usePositionClass } from "@/components/window/is-windowed";

function Toast({ msg, color, onDone }) {
  const positionClass = usePositionClass();
  useEffect(() => { const t = setTimeout(onDone, 2800); return () => clearTimeout(t); }, []);
  return (
    <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 24 }}
      className={`${positionClass} bottom-8 left-1/2 -translate-x-1/2 z-50 font-mono text-sm px-5 py-3 rounded border pointer-events-none`}
...
export function MissionsApp() {
  ...
  const handleAccept = async (m) => {
    ...
    try {
      ...
      await updateDoc(doc(db, "missions", m.id), { filled: increment(1) });
