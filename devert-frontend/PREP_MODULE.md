...
1. Grant yourself the platform admin claim: `node scripts/set-admin-claim.mjs <email>` - the same way every other admin surface bootstraps (see root `CLAUDE.md`). This
...
enforced both client-side and in `firestore.rules`) and is immutable once set - staff can correct a wrong one via `/prep/admin` (`staffUpdateUser`).
