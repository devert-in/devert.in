...
// A second, much smaller output: the catalogue the FRONTEND compiles against.
//
// Metadata for the 100 day pages has to exist at build time, and it must not
// depend on reaching Firestore - a build that cannot resolve the network would
// otherwise emit 100 pages with no title, silently. Everything SEO needs is
// static content that changes only when the spreadsheet does, so it ships as a
// source file rather than a fetch. ~10KB, tree-shaken to nothing at runtime
// because only generateMetadata reads it.
const CATALOG_OUT = "devert-frontend/lib/devert100Catalog.js";
const catalog = days.map(d => {
  ...
  return {
    ...
    // One clean sentence for the meta description. The sheet's statements are
    // already short and declarative, so they need trimming, not rewriting.
    blurb: (main?.statement || "").replace(/\s+/g, " ").trim(),
  };
});
...
writeFileSync(CATALOG_OUT, [
  ...
  "export const DEVERT100_CATALOG = " + JSON.stringify(catalog, null, 2) + ";",
  "",
...
console.log(`wrote ${CATALOG_OUT}  (${catalog.length} entries)`);
