// Read-only audit: how much of the proposed "DeVert Interview Sheet" can
// actually be built by REFERENCE against the existing `problems` collection?
//
// The sheet spec's central rule is "never duplicate a problem - reference its
// existing problem ID". That rule is only satisfiable for problems DeVert
// already has. This script answers, per section, which entries resolve to a
// real problem doc and which would need original content authored first, so
// the sheet is scoped against reality instead of an assumed 100% hit rate.
//
// Writes nothing. Run: node scripts/audit-interview-sheet-coverage.mjs
...
// The proposed sheet, transcribed verbatim from the spec: [leetcodeNumber, title].
// LeetCode numbers are recorded only as a human cross-reference for whoever
// reconciles a miss by hand - matching below is by TITLE, since DeVert's own
// problem docs carry no external-id field.
const SHEET = {
  "Arrays": [
...
// Aggressive normalization so cosmetic authoring differences ("Sqrt(x)" vs
// "Sqrt x", "Merge K" vs "Merge k", "strStr()" vs "strStr") don't read as a
// missing problem. Roman-numeral suffixes are PRESERVED - "Single Number" and
// "Single Number II" are different problems and must never collapse together.
...
// Exact-title matching alone is far too strict for this database: DeVert's
// problems are authored with descriptive names of their own, so LC 206
// "Reverse Linked List" lives here as "Reverse a Linked List", LC 155 "Min
// Stack" as "Min Stack - Support getMin in O(1)", LC 103 as "Zig-Zag Level
// Order Traversal". Matching on exact strings reported Linked Lists and Trees
// as 0% present when in fact both are well covered.
//
// So: score every candidate by token overlap and emit a REVIEWABLE mapping
// rather than pretending this can be decided automatically. Deliberately not
// auto-accepting anything below CONFIRMED - "Merge Two Sorted Lists" vs
// "Merge Two Binary Max-Heaps" share 3 of 4 tokens and are unrelated problems,
// so a human has to sign off on the LIKELY/REVIEW tiers before any sheet
// references them.
...
function similarity(sheetTitle, dbTitle) {
  const a = tokens(sheetTitle);
  const b = tokens(dbTitle);
  if (a.size === 0 || b.size === 0) return 0;
  let shared = 0;
  for (const t of a) if (b.has(t)) shared++;
  const coverage = shared / a.size;              // how much of the sheet title is present
  const noise = (b.size - shared) / Math.max(b.size, 1); // how much extra the candidate carries
  return coverage - noise * 0.25;
}

function classify(score, exact) {
...
async function main() {
  ...
  console.log(`DeVert problems collection: ${problems.length} docs `
    + `(${problems.filter(p => p.published).length} published)\n`);

  const tally = { CONFIRMED: 0, LIKELY: 0, REVIEW: 0, UNRESOLVED: 0 };
  const mapping = {};
  const matchedIds = new Set();
  let entries = 0;
  const uniqueTitles = new Set();

  for (const [section, list] of Object.entries(SHEET)) {
    mapping[section] = [];
    const perTier = { CONFIRMED: 0, LIKELY: 0, REVIEW: 0, UNRESOLVED: 0 };

    for (const [lc, title] of list) {
      entries++;
      uniqueTitles.add(norm(title));
      ...
      const tier = classify(bestScore, !!exact?.length);
      tally[tier]++; perTier[tier]++;
      if (tier !== "UNRESOLVED") matchedIds.add(best.id);

      mapping[section].push({
        leetcode: lc, sheetTitle: title, tier,
        score: Number(bestScore.toFixed(2)),
        problemId: tier === "UNRESOLVED" ? null : best.id,
        dbTitle: tier === "UNRESOLVED" ? null : best.title,
        difficulty: tier === "UNRESOLVED" ? null : best.difficulty,
        dbCategory: tier === "UNRESOLVED" ? null : best.category,
      });
    }

    const usable = perTier.CONFIRMED + perTier.LIKELY;
    console.log(`=== ${section}: ${usable}/${list.length} usable `
      + `(confirmed ${perTier.CONFIRMED}, likely ${perTier.LIKELY}, `
      + `review ${perTier.REVIEW}, unresolved ${perTier.UNRESOLVED}) ===`);
    mapping[section].filter(m => m.tier === "REVIEW").forEach(m =>
      console.log(`   REVIEW      LC ${m.leetcode} "${m.sheetTitle}"  ->  ${m.dbTitle} [${m.score}]`));
    mapping[section].filter(m => m.tier === "UNRESOLVED").forEach(m =>
      console.log(`   UNRESOLVED  LC ${m.leetcode} "${m.sheetTitle}"`));
  }

  const withCompanies = problems.filter(p => matchedIds.has(p.id) && p.companies.length > 0).length;

  console.log(`\n---------------- SUMMARY ----------------`);
  console.log(`DeVert problems available:                 ${problems.length}`);
  console.log(`Sheet entries (incl. cross-section repeats): ${entries}`);
  console.log(`Distinct problems the sheet refers to:     ${uniqueTitles.size}`);
  console.log(`  CONFIRMED  (exact title match):          ${tally.CONFIRMED}`);
  console.log(`  LIKELY     (>=0.85, spot-check):         ${tally.LIKELY}`);
  console.log(`  REVIEW     (0.55-0.85, human decides):   ${tally.REVIEW}`);
  console.log(`  UNRESOLVED (needs authoring):            ${tally.UNRESOLVED}`);
  console.log(`Distinct problem docs referenced:          ${matchedIds.size}`);
  console.log(`Of those, already carry company tags:      ${withCompanies}/${matchedIds.size}`);

  const out = path.join(__dirname, "interview-sheet-mapping.draft.json");
  const { writeFileSync } = await import("fs");
  writeFileSync(out, JSON.stringify(mapping, null, 2));
  console.log(`\nDraft mapping written to scripts/interview-sheet-mapping.draft.json`);
  console.log(`Nothing was written to Firestore. Review the LIKELY/REVIEW tiers before any seeding.`);
