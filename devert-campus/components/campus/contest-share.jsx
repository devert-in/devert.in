...
//
// THE URL. /{slug}/contest/{id} is the app's own canonical deep link, not
// one invented here: campus-app.jsx parses `second === "contest" ? third`
// into initialContestId on load, and writes exactly this shape back via
// replaceState when a contest opens. The campus Hosting site's own preview-
// router rewrite serves the campus SPA shell for any path under it, so the
// link resolves in production even though there is no pre-rendered Next
// route for the [contestId] segment.
//
...
const contestPath = (slug, contestId) => `/${slug}/contest/${contestId}`;

// Every Campus URL is /{slug}/... - both the workspace root
// (/mrcet?tab=contests) and the deep link (/mrcet/contest/{id}) put the
// slug in the same position, so reading it back is reliable from either.
// Derived rather than threaded through as a prop because
...
export function slugFromPath(pathname) {
  ...
  // No leading "campus" segment on campus.devert.in - the institution slug
  // is the first path segment.
  return parts[0] || "";
}

export function contestShareUrl(slug, contestId, origin) {
  const base = origin || (typeof window !== "undefined" ? window.location.origin : "https://campus.devert.in");
