...
export const LANDING_PAGE_SECTIONS = ["campuses", "institutions", "pricing", "daily-learning", "assessments"];
...
export function buildCampusMetadata(inst, sectionKey) {
  ...
  const path = section.urlSegment ? `/${inst.id}/${section.urlSegment}` : `/${inst.id}`;
  const url = `${SITE}${path}`;
...
export function campusJsonLd(inst, sectionKey) {
  ...
  const breadcrumbItems = [
    { name: "Home", item: SITE },
    { name: "Campus", item: SITE },
    { name: inst.name, item: `${SITE}/${inst.id}` },
  ];
...
export function campusBreadcrumbItems(inst, sectionKey) {
  ...
  const items = [
    { label: "Home", href: "/" },
    { label: "Campus", href: "/" },
    { label: inst.name, href: section.urlSegment ? `/${inst.id}` : undefined },
  ];
